...
type HttpMethod = {GET:string, POST:string, PUT:string, PATCH:string, DELETE:string};
...
type requete = {method:HttpMethod, url:string, params?:string[], query?:string|Record<string, string>, body?:Record<string, unknown>, headers:{"Content-Type": string} & Record<string, unknown>};
...
type Guard = {canActivate: (request:Request) => boolean | Promise<boolean>};
...
type Interceptor = {intercept: (request:Request) => void};
...
type ValidationSchema = {fields:Record<string, Record<string, unknown>>, required?:string[]};
...
type Route = {path:string, method:HttpMethod, handler:string, guards?:Guard[], validationSchema?:ValidationSchema, requestInterceptors?:Interceptor[], responseInterceptor?:Interceptor[]};
...
type reponse = {statusCode:number, headers:{"Content-Type": string} & Record<string, unknown>, body?: Record<string, unknown>}
