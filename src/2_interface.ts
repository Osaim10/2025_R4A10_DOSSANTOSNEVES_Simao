...
interface Create<T> {
    create: (nv_entite:T) => Promise<T | null>;
}

interface Update<T> {
    update: (id:number, nv_valeur:T) => Promise<T | null>;
}
...
type Student = {id:number, firstName:string, lastName:string, group:string};
type Course = {id:number, name:string, enseignant:string, active:boolean};
...
class SQLStudentRepository implements StudentRepository {
    async findMany(): Promise<Student[]> { return []; }
    async findOne(id:number): Promise<Student | null> { return null; }
    async create(nv_entite:Student): Promise<Student | null> { return null; }
    async update(id:number, nv_valeur:Student): Promise<Student | null> { return null; }
    async delete(id:number): Promise<void> {}
}

class SQLCourseRepository implements CourseRepository {
    async findMany(): Promise<Course[]> { return []; }
    async findOne(id:number): Promise<Course | null> { return null; }
    async create(nv_entite:Course): Promise<Course | null> { return null; }
    async update(id:number, nv_valeur:Course): Promise<Course | null> { return null; }
}
